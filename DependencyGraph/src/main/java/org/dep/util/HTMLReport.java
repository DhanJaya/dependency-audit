...
/**
 * Class which helps generate the HTML reports generated by the tool
 */
public class HTMLReport {
    ...
    public static void generateDependencyDetailsHTML(String projectName, Graph<Node, DefaultEdge> dependencyTree, Map<Node, String> hrefTransitiveMap, Map<String, Set<Reference>> allUnMappedReferences) throws IOException {
        Document doc = Document.createShell("");
        ...
        appendTableBody(dependencyTree, hrefTransitiveMap, table);
    ...
    private static void appendTableBody(Graph<Node, DefaultEdge> dependencyTree, Map<Node, String> hrefTransitiveMap, Element table) {
        ...
        while (iterator.hasNext()) {
            ...
            for (DefaultEdge edge : dependencyTree.edgesOf(node)) {
                if (!visitedEdges.contains(edge)) {
                    ...
                    Map<String, Set<Reference>> classAndReferences = dependencyTree.getEdgeTarget(edge).getReferences();
                    formatReferenceString(referencesString, classAndReferences, "<br>");
    ...
    /**
     * Format the references invoked the project to display it in the HTML page and the CSV file
     * @param referencesString The string to append the references to
     * @param classAndReferences All classes and their references to be appended
     * @param LineBreak The line break that should be used
     */
    public static void formatReferenceString(StringBuilder referencesString, Map<String, Set<Reference>> classAndReferences, String LineBreak) {
        for (Map.Entry<String, Set<Reference>> entry : classAndReferences.entrySet()) {
            ...
            for (Reference reference : references) {
                if (reference.getInstruction() != null) {
                    referencesString.append(reference.getInstruction()).append(" -> ").append(className).append("::").append(reference.getName()).append(LineBreak);
                } else {
                    referencesString.append(className).append("::").append(reference.getName()).append(LineBreak);
                }
            }
            if (references.isEmpty()) {
                referencesString.append(className).append(LineBreak);
            }
        }
    }
    ...
    private static void appendTableHeader(Element table) {
        ...
        headerRow.appendElement("th").text("Bloated Direct Dependency");
